...
public interface EntryTransactionRepository extends CrudRepository<EntityTransaction, String> {
    ...
    @Query(value = "SELECT sum(deposit - withdraw) FROM entry_transaction WHERE user_role = 'DRIVER'AND driver_id=:driverId AND createdDate > :fromDate ", nativeQuery = true)
    long getDriverBalanceFromDate(@Param("driverId") String driverId, @Param("fromDate") Date date);

    @Query(value = "SELECT sum(withdraw) FROM entry_transaction  WHERE user_id=:driverId AND type='DRIVER_SETTLE' AND createdDate > :fromDate", nativeQuery = true)
